...
# a <- readRDS('/celltype/mca/data/proc/anno.rds')
...
rt <- system.time({
	...
	clu <- a[match(colnames(es.max),a[,1]),'celltype']
	ss <- rowsum(t(es.max),clu)
	ct <- colnames(ss)[apply(ss,1,which.max)]
	ct <- data.frame(orict=rownames(ss),predct=ct)
})
# saveRDS(rt,file='/celltype/mca/sctype/time.rds')
# saveRDS(ct,file='/celltype/mca/sctype/res.rds')
